...
expName = 'vSearchAlpha4'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\Andrew\\Documents\\Work\\Projects\\v-search-alpha4\\vSearchAlpha4_lastrun.py',
...
instr1 = visual.TextStim(win=win, name='instr1',
    text="In this task you will see sets of shapes made of four different shapes as outlined below. Only press the 'b' key if you see the “T” shape, which is the last shape. Otherwise, press the 'm' key. There will be 6 shapes to observe. Press space to continue.",
    font='Arial',
...
breakText1 = visual.TextStim(win=win, name='breakText1',
    text='A quick break before the next set of trials. The next set will have 12 shapes to observe. Press space to continue.',
    font='Arial',
    pos=(0, 0), height=0.05, wrapWidth=None, ori=0, 
    color='white', colorSpace='rgb', opacity=1, 
    languageStyle='LTR',
...
breakText2 = visual.TextStim(win=win, name='breakText2',
    text='A quick break before the next set of trials. The next set will have 18 shapes to observe. Press space to continue.',
    font='Arial',
    pos=(0, 0), height=0.05, wrapWidth=None, ori=0, 
    color='white', colorSpace='rgb', opacity=1, 
    languageStyle='LTR',
...
for thisTrials1 in trials1:
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if trial1Resp.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > trial1Resp.tStartRefresh + 3.0-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if trial1Resp.status == STARTED and not waitOnFlip:
            theseKeys = trial1Resp.getKeys(keyList=['b', 'm'], waitRelease=False)
...
for thisTrials2 in trials2:
    ...
    trial2TT.setPos([loc25,loc26])
    trial2Resp.keys = []
    trial2Resp.rt = []
    _trial2Resp_allKeys = []
    # keep track of which components have finished
    trial2Components = [trial2Fix, trial2L11, trial2L12, trial2L21, trial2L22, trial2T11, trial2T12, trial2L13, trial2L14, trial2L23, trial2L24, trial2T13, trial2T14, trial2TT, trial2Resp]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if trial2L13.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > trial2L13.tStartRefresh + 3.0-frameTolerance:
                # keep track of stop time/frame for later
                trial2L13.tStop = t  # not accounting for scr refresh
                trial2L13.frameNStop = frameN  # exact frame index
                win.timeOnFlip(trial2L13, 'tStopRefresh')  # time at next scr refresh
                trial2L13.setAutoDraw(False)
        ...
        if trial2L14.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > trial2L14.tStartRefresh + 3.0-frameTolerance:
                # keep track of stop time/frame for later
                trial2L14.tStop = t  # not accounting for scr refresh
                trial2L14.frameNStop = frameN  # exact frame index
                win.timeOnFlip(trial2L14, 'tStopRefresh')  # time at next scr refresh
                trial2L14.setAutoDraw(False)
        ...
        if trial2L23.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > trial2L23.tStartRefresh + 3.0-frameTolerance:
                # keep track of stop time/frame for later
                trial2L23.tStop = t  # not accounting for scr refresh
                trial2L23.frameNStop = frameN  # exact frame index
                win.timeOnFlip(trial2L23, 'tStopRefresh')  # time at next scr refresh
                trial2L23.setAutoDraw(False)
        ...
        if trial2L24.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > trial2L24.tStartRefresh + 3.0-frameTolerance:
                # keep track of stop time/frame for later
                trial2L24.tStop = t  # not accounting for scr refresh
                trial2L24.frameNStop = frameN  # exact frame index
                win.timeOnFlip(trial2L24, 'tStopRefresh')  # time at next scr refresh
                trial2L24.setAutoDraw(False)
        ...
        if trial2T13.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > trial2T13.tStartRefresh + 3.0-frameTolerance:
                # keep track of stop time/frame for later
                trial2T13.tStop = t  # not accounting for scr refresh
                trial2T13.frameNStop = frameN  # exact frame index
                win.timeOnFlip(trial2T13, 'tStopRefresh')  # time at next scr refresh
                trial2T13.setAutoDraw(False)
        ...
        if trial2T14.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > trial2T14.tStartRefresh + 3.0-frameTolerance:
                # keep track of stop time/frame for later
                trial2T14.tStop = t  # not accounting for scr refresh
                trial2T14.frameNStop = frameN  # exact frame index
                win.timeOnFlip(trial2T14, 'tStopRefresh')  # time at next scr refresh
                trial2T14.setAutoDraw(False)
        ...
        if trial2Resp.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > trial2Resp.tStartRefresh + 3.0-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if trial2Resp.status == STARTED and not waitOnFlip:
            theseKeys = trial2Resp.getKeys(keyList=['b', 'm'], waitRelease=False)
...
for thisTrials3 in trials3:
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if trial3L13.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > trial3L13.tStartRefresh + 3.0-frameTolerance:
                # keep track of stop time/frame for later
                trial3L13.tStop = t  # not accounting for scr refresh
                trial3L13.frameNStop = frameN  # exact frame index
                win.timeOnFlip(trial3L13, 'tStopRefresh')  # time at next scr refresh
                trial3L13.setAutoDraw(False)
        ...
        if trial3L14.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > trial3L14.tStartRefresh + 3.0-frameTolerance:
                # keep track of stop time/frame for later
                trial3L14.tStop = t  # not accounting for scr refresh
                trial3L14.frameNStop = frameN  # exact frame index
                win.timeOnFlip(trial3L14, 'tStopRefresh')  # time at next scr refresh
                trial3L14.setAutoDraw(False)
        ...
        if trial3L23.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > trial3L23.tStartRefresh + 3.0-frameTolerance:
                # keep track of stop time/frame for later
                trial3L23.tStop = t  # not accounting for scr refresh
                trial3L23.frameNStop = frameN  # exact frame index
                win.timeOnFlip(trial3L23, 'tStopRefresh')  # time at next scr refresh
                trial3L23.setAutoDraw(False)
        ...
        if trial3L24.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > trial3L24.tStartRefresh + 3.0-frameTolerance:
                # keep track of stop time/frame for later
                trial3L24.tStop = t  # not accounting for scr refresh
                trial3L24.frameNStop = frameN  # exact frame index
                win.timeOnFlip(trial3L24, 'tStopRefresh')  # time at next scr refresh
                trial3L24.setAutoDraw(False)
        ...
        if trial3T13.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > trial3T13.tStartRefresh + 3.0-frameTolerance:
                # keep track of stop time/frame for later
                trial3T13.tStop = t  # not accounting for scr refresh
                trial3T13.frameNStop = frameN  # exact frame index
                win.timeOnFlip(trial3T13, 'tStopRefresh')  # time at next scr refresh
                trial3T13.setAutoDraw(False)
        ...
        if trial3T14.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > trial3T14.tStartRefresh + 3.0-frameTolerance:
                # keep track of stop time/frame for later
                trial3T14.tStop = t  # not accounting for scr refresh
                trial3T14.frameNStop = frameN  # exact frame index
                win.timeOnFlip(trial3T14, 'tStopRefresh')  # time at next scr refresh
                trial3T14.setAutoDraw(False)
        ...
        if trial3L15.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > trial3L15.tStartRefresh + 3.0-frameTolerance:
                # keep track of stop time/frame for later
                trial3L15.tStop = t  # not accounting for scr refresh
                trial3L15.frameNStop = frameN  # exact frame index
                win.timeOnFlip(trial3L15, 'tStopRefresh')  # time at next scr refresh
                trial3L15.setAutoDraw(False)
        ...
        if trial3L16.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > trial3L16.tStartRefresh + 3.0-frameTolerance:
                # keep track of stop time/frame for later
                trial3L16.tStop = t  # not accounting for scr refresh
                trial3L16.frameNStop = frameN  # exact frame index
                win.timeOnFlip(trial3L16, 'tStopRefresh')  # time at next scr refresh
                trial3L16.setAutoDraw(False)
        ...
        if trial3L25.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > trial3L25.tStartRefresh + 3.0-frameTolerance:
                # keep track of stop time/frame for later
                trial3L25.tStop = t  # not accounting for scr refresh
                trial3L25.frameNStop = frameN  # exact frame index
                win.timeOnFlip(trial3L25, 'tStopRefresh')  # time at next scr refresh
                trial3L25.setAutoDraw(False)
        ...
        if trial3L26.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > trial3L26.tStartRefresh + 3.0-frameTolerance:
                # keep track of stop time/frame for later
                trial3L26.tStop = t  # not accounting for scr refresh
                trial3L26.frameNStop = frameN  # exact frame index
                win.timeOnFlip(trial3L26, 'tStopRefresh')  # time at next scr refresh
                trial3L26.setAutoDraw(False)
        ...
        if trial3T15.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > trial3T15.tStartRefresh + 3.0-frameTolerance:
                # keep track of stop time/frame for later
                trial3T15.tStop = t  # not accounting for scr refresh
                trial3T15.frameNStop = frameN  # exact frame index
                win.timeOnFlip(trial3T15, 'tStopRefresh')  # time at next scr refresh
                trial3T15.setAutoDraw(False)
        ...
        if trial3T16.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > trial3T16.tStartRefresh + 3.0-frameTolerance:
                # keep track of stop time/frame for later
                trial3T16.tStop = t  # not accounting for scr refresh
                trial3T16.frameNStop = frameN  # exact frame index
                win.timeOnFlip(trial3T16, 'tStopRefresh')  # time at next scr refresh
                trial3T16.setAutoDraw(False)
        ...
        if trial3Resp.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > trial3Resp.tStartRefresh + 3.0-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if trial3Resp.status == STARTED and not waitOnFlip:
            theseKeys = trial3Resp.getKeys(keyList=['b', 'm'], waitRelease=False)
